...
if [ -d $InDir ]; then
    ...
    NumbFiles=$(find $InDir/ -type f ! -name "histProbFunction.root" -name "*.root" | wc -l)
else
    ...
    NumbFiles=$(find $inDir/ -type f  ! -name "histProbFunction.root" -name "$prefix*.root" | wc -l)
#    echo $NumbFiles
fi
...
qsub -t 1-$NumbJobs MiniAODjob.sh $InDir
#SumEvents=$(())
